import { Property, VariantsOfUnion, TypeExt, UnionHandler } from "./types";
 ...
 */
type WithDefault<T, U = any> = (Partial<T> & { default: (...args: Parameters<FuncsOnly<T>[keyof T]>) => U }) | T;

/**
 * Pick just the functions of an object.
 */
type FuncsOnly<T> = {
	[P in keyof T]: T[P] extends Callback ? T[P] : never;
};
 ...
 */
export function match<T extends Property<"type", string>, H extends WithDefault<Handler<VariantsOfUnion<T>>>>(
	obj: T,
	handler: H & Limited<H, T["type"] | DEFAULT_KEY>,
...
>(
	obj: T,
	handler: H & Limited<H, T[K] | DEFAULT_KEY>,
	typeKey?: K,
): ReturnType<Limit<FuncsOnly<H>, T[K] | DEFAULT_KEY>[keyof H]>;
/**
...
>(obj: T, handler: H, _elseOrKey?: E | K, key?: K) {
	const typeKey = typeIs(_elseOrKey, "string") ? _elseOrKey : key;
	const typeString = obj[(typeKey as string | undefined) ?? ("type" as K)];

	if (typeString in handler) {
		const handlerFn = handler[typeString as keyof typeof handler];
		return handlerFn?.(obj as any);
	} else {
		if (_elseOrKey !== undefined && typeIs(_elseOrKey, "function")) {
			return _elseOrKey(obj as any);
		} else {
			if ("default" in handler) {
				return (handler as { default(this: void, t: T): any }).default(obj);
			}
			return undefined;
		}
	}
...
export function matchLiteral<T extends string, H extends UnionHandler<T>>(literal: T, handler: H): ReturnType<H[T]> {
	const lit = handler[literal] as (variant: string) => ReturnType<H[T]>;
	return lit?.(literal);
}
